...
static void HandleSdlEvent(const SDL_Event& event, SdlAppRuntime& runtime) {
	...
	if (event.type == SDL_EVENT_TEXT_EDITING) {
		InputEventPacket packet = CreateBaseInputPacket(runtime, InputEventType::Composition);
		packet.composition_start = event.edit.start;
		packet.composition_length = event.edit.length;
		if (event.edit.text) {
			std::strncpy(packet.text, event.edit.text, sizeof(packet.text) - 1);
			packet.text[sizeof(packet.text) - 1] = '\0';
		}
		SendInputPacket(runtime.input_pipe, packet);
		return;
